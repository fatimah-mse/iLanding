...
import close from '../images/x-white.png'
import menu from '../images/IconNav.png'
import menuScroll from '../images/IconNavScroll.png'
...
export default function Navbar() {
    ...
    return (
        <nav className={`${header ? 'bg-[#92b5db] w-screen px-20 max-[1366px]:px-14 max-[768px]:px-5 top-0' : 'bg-white w-[calc(100vw-160px)] max-[1366px]:w-[calc(100vw-112px)] max-[768px]:w-[calc(100vw-40px)] max-[1366px]:left-14 max-[768px]:left-5 left-20 top-5 px-6 rounded-full'} py-3 max-[992px]:py-2 flex justify-between items-center fixed z-40 transition-all duration-600 ease-in-out`}>
            <Link href="/" className={`${header ? 'text-white' : 'text-[#f15936]'} text-2xl font-bold `}>iLanding</Link>
            <div className={`relative max-[992px]:bg-[#92b5db] backdrop-blur-xl max-[992px]:w-screen max-[992px]:h-screen transition-all duration-300 ease-in-out flex items-center lg:justify-between max-[992px]:justify-evenly max-[992px]:fixed max-[992px]:left-0 max-[992px]:top-0 max-[992px]:flex-col max-[992px]:items-center ${show ? 'translate-x-0' : 'max-[992px]:-translate-x-full'}`}>
                <Link href="/" className='text-white font-bold text-4xl hidden max-[992px]:block'>iLanding</Link>
            ...
            <Image onClick={() => setShow(!show)} className='hidden max-[992px]:block w-8 h-8 cursor-pointer' src={header ? menuScroll : menu} alt="menu" />
        </nav>
    )
}
